...
@Component({
  selector: 'app-copyrequest',
  templateUrl: './copyrequest.component.html',
  styleUrls: ['./copyrequest.component.css']
})
export class CopyrequestComponent implements OnInit {
      ...
      .subscribe((data: any) => {
        if (data.success) {
          ...
          this.keys.forEach((element) => {
            const label = this.requests[0].labels[0][element];
  ...
  pdfExport() {
    // tslint:disable-next-line:max-line-length
    const title: string = this.title === null ? 'Reporte de solicitudes' : this.title === undefined ? 'Reporte de solicitudes' : this.title === '' ? 'Reporte de solicitudes' : this.title;

...
import {
  Travel,
  Travel_managements,
} from '../../../../models/common/travels_management/travel/travel';
...
export class EditTravelComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.alert.getActionConfirm().subscribe((data: any) => {
      ...
      if (data === 'deleteDocumentSaved') {
        ...
        this.travelManagementService
          .deleteFile(this.idFile.toString(), this.ticket)
          .subscribe(() => {
            this.generalViajes[0].travel_request_annexeds.splice(
              this.generalViajes[0].travel_request_annexeds.findIndex(
                filter => filter.id === this.idFile,
              ),
              1,
            );
          });
      ...
      if (data === 'deleteDestinations') {
        ...
        if (this.acctionDeleteTable === 'deleteTravelManagement') {
          this.travelManagementService
            .deleteTravelByDestination(this.ticketDestinations, this.ticket)
            .subscribe(() => {
              this.generalViajes[0].travel_managements.data.splice(
                this.generalViajes[0].travel_managements.data.findIndex(
                  filter => filter.field_0 === this.id_destination_delete,
                ),
                1,
              );
              this.traverlsDestination.splice(
                this.traverlsDestination.findIndex(
                  filter => filter.travel_id === this.id_destination_delete,
                ),
                1,
              );
              setTimeout(() => {
        ...
        this.travelManagementService
          ...
          .subscribe((tavelManagement: any) => {
            ...
            this.travel_types = this.sortByAphabet(
              tavelManagement.data.travel_types,
            );
            this.transport_types = this.sortByAphabet(
              tavelManagement.data.transport_types,
            );
            this.countries = this.sortByAphabet(tavelManagement.data.countries);
            this.countriesto = this.sortByAphabet(
              tavelManagement.data.countries,
            );
            this.legal_travels = this.sortByAphabet(
              tavelManagement.data.legal_travels_types,
            );
            this.trips_specific = this.sortByAphabet(
              tavelManagement.data.specific_types_trips,
            );
            this.trips_activities = this.sortByAphabet(
              tavelManagement.data.travel_activities,
            );
            this.center_costs_travels = this.sortByAphabet(
              tavelManagement.data.travel_costs_types,
            );
            this.costs_travels = [];
            ...
            this.travelManagementService
              ...
              .subscribe((result: any) => {
                if (result.success) {
                  ...
                  this.comentaryPlus =
                    result.data[0].travel_request.commentary !== null
                  ...
                  this.formTravelManagement = new FormGroup({});
                  this.formTravelManagement = fb.group({
                    type_travel: this.generalViajes[0].travel_request
                      .travel_type_id,
                    date_requests_begin: this.generalViajes[0].travel_request
                      .date_begin,
                    date_requests_end: this.generalViajes[0].travel_request
                      .date_end,
                    trip_text: this.generalViajes[0].travel_request.observation,
                    maintenance: this.generalViajes[0].travel_request
                      .is_maintenance,
                    id_element_imputation: this.generalViajes[0].travel_request
                      .travel_costs_type_id,
                    id_grahp: this.generalViajes[0].travel_request
                      .travel_graph_code,
                    id_operations: this.generalViajes[0].travel_request
                      .travel_operation_id,
                    id_travel_costs: this.generalViajes[0].travel_request
                      .travel_cost_id,
                    id_travel_legal: this.generalViajes[0].travel_request
                      .legal_travels_type_id,
                    id_travel_specific: this.generalViajes[0].travel_request
                      .specific_types_trip_id,
                    id_travel_activities: this.generalViajes[0].travel_request
                      .travel_activity_id,
                    name_travel_graph:
                      this.generalViajes[0].travel_request.travel_graph_code ===
                      null
                        ? ''
                        : this.generalViajes[0].travel_request
                            .travel_graph_code +
                          ' - ' +
                          this.generalViajes[0].travel_request
                            .travel_graph_name,
                    ...
                    name_travel_order:
                      ...
                      null
                        ...
                        : this.generalViajes[0].travel_request
                          ...
                          this.generalViajes[0].travel_request
                            .travel_order_name,
                    id_order: this.generalViajes[0].travel_request
                      .travel_maintenance_order_id,
                  });

                  if (
                    result.data[0].travel_request.employee_applicant_to_json !==
                    null
                  ) {
                  ...
                  setTimeout(() => {
                    ...
                    if (
                      ...
                      this.searchOperationsGrahp(
                        this.formTravelManagement.value.id_grahp,
                        '',
                      );
                    }
                  }, 1000);
    ...
    this.accionDataTableService.getActionDataTable().subscribe((data: any) => {
      ...
      if (data.action_method === 'updateHotels') {
        const date_requests_begin = this.generalViajes[0].travel_request
          .date_begin;
          ...
          this.travelManagementService
            ...
            .subscribe((resutlDestinations: any) => {
              ...
              this.formTravelManagementedit = {
                ...
                id_terminal:
                  resutlDestinations.data.origin_transport_terminal_id,
                date_begin: this.split_begin[0],
                hour_begin: this.split_begin[1],
                hour_end: this.split_end[1],
                date_end: this.split_end[0],
                id_terminalto:
                  resutlDestinations.data.destination_transport_terminal_id,
      ...
      if (data.action_method === 'updateTrayectManagement') {
        ...
        if (this.bedit === true) {
          const object: any = this.editEditTrip.filter(
  ...
  addHourEnd(value) {
    ...
    const calculate_minutes =
      end.toString().length === 1
        ? '0' + end.toString()
        : end > 59
        ? '00'
        : end.toString();
    ...
    this.formTravelManagement.controls['hour_end'].setValue(
      calculate_hour + ':' + calculate_minutes,
    );
  }
  ...
  enterOrder(form) {
    this.travelManagementService
      .getFilterTravelOrders(form.name_travel_order.toUpperCase())
  ...
  delete(date_param) {
    ...
    switch (date_param) {
      case 'date_begin_header':
        controls['date_requests_begin'].setValue(
          this.generalViajes[0].travel_request.date_begin,
        );
        break;
      case 'date_end_header':
        controls['date_requests_end'].setValue(
          this.generalViajes[0].travel_request.date_end,
        );
        break;
    ...
    this.alertWarning = [
      {
        ...
        message:
          '¿' + this.t('message_alert_ts') + this.idFile.toString() + '?',
        confirmation: true,
    ...
    this.alertWarning = [
      {
        ...
        message:
          '¿' +
          this.t('message_alert_one_ts') +
          this.id_destination_delete.toString() +
          '?',
        confirmation: true,
  ...
  newEditTravel(model) {
    ...
    modelFromdata.append(
      'legal_travels_type_id',
      model.id_travel_legal == '-1' ? '' : model.id_travel_legal,
    );
    modelFromdata.append(
      'specific_types_trip_id',
      model.id_travel_specific == '-1' ? '' : model.id_travel_specific,
    );
    modelFromdata.append(
      'travel_activity_id',
      model.id_travel_activities == '-1' ? '' : model.id_travel_activities,
    );
    modelFromdata.append(
      'travel_cost_id',
      model.id_travel_costs == '-1' ? '' : model.id_travel_costs,
    );
    modelFromdata.append(
      'date_begin',
      model.date_requests_begin == '-1' ? '' : model.date_requests_begin,
    );
    modelFromdata.append(
      'date_end',
      model.date_requests_end == '-1' ? '' : model.date_requests_end,
    );
    ...
    modelFromdata.append(
      'travel_maintenance_order_id',
      model.id_order == '-1' ? '' : model.id_order,
    );
    modelFromdata.append(
      'employee_id',
  ...
  editDestination(modelEditPartial) {
      ...
      if (element.field_0.toString() === this.id_destinations.toString()) {
        (element.field_1 =
          ...
          (element.field_3 = this.terminalLocations.filter(
            data =>
              data.id.toString() === modelEditPartial.id_terminal.toString(),
          )[0].name);
        ...
        (element.field_5 = modelEditPartial.id_cityto),
          (element.field_6 = this.terminalLocationsto.filter(
            data =>
              data.id.toString() === modelEditPartial.id_terminalto.toString(),
          )[0].name);
    ...
    this.traverlsDestination.splice(
      this.traverlsDestination.findIndex(
        filter => filter.travel_id === this.id_destinations,
      ),
      1,
    );
    ...
    this.traverlsDestination.push({
      ...
      origin_datetime:
        modelEditPartial.date_begin + ' ' + modelEditPartial.hour_begin,
      destination_datetime:
        modelEditPartial.date_end + ' ' + modelEditPartial.hour_end,
  ...
  mileageTravel(param) {
    if (
      this.transport_types.filter(
        data => data.id.toString() === param.id_transport.toString(),
      )[0].cttype == 'T'
    ) {
      this.showMilenage = true;
    } else {
  ...
  searchStateto(form: any, acction: any) {
    ...
    this.travelManagementService
      .getgeographicLocations(form.id_countryto)
      .subscribe((data: any) => {
  ...
  searchTerminal(form: any, acction: any) {
    this.terminalLocations = [];
    this.travelManagementService
      .gettransportTerminals(form.id_country)
      .subscribe((data: any) => {
  ...
  searchTerminalto(form: any, acction: any) {
    this.terminalLocationsto = [];
    this.travelManagementService
      .gettransportTerminals(form.id_countryto)
      .subscribe((data: any) => {
  ...
  searchCostsCenterAndGrahp(form: any) {
    if (
      ...
      const employee_center_coast =
        this.eployee_selected === null
          ? this.userAuthenticated.employee.cost_center
          : this.eployee_selected.cost_center;
            ...
            const alertDataWrong: Alerts[] = [
              {
                ...
                message:
                  error.json().errors.toString() +
                  this.t('message_alert_six_ts'),
                confirmation: true,
                typeConfirmation:
                  'continueEditDestinationRequestsValidateDates',
              },
            ];
            this.alert.setAlert(alertDataWrong[0]);
          },

...
export class ViewSpendComponent implements OnInit {
  ...
  constructor(
    ...
    this.accionDataTableServiceView
      .getActionDataTable()
      .subscribe((data: any) => {
    ...
    this.spendSharedService.getViewSpend().subscribe((idSpend: any) => {
      this.spendsService
        .getViewDetailSpends(idSpend, this.edit)
        .subscribe((data: any) => {
          ...
          this.showTravelDetail =
            data.data[0].travel_allowance_request.info_travel;
          ...
          setTimeout(() => {
            this.objectReport.emit({
              data: [data.data[0].travel_allowances],
            });
          }, 500);

          if (
            document.getElementById('modal_viewSpends').className !==
            'modal show'
          ) {
          ...
          this.travelManagementService
            ...
            .subscribe((detail: any) => {
              ...
              if (detail.data[0].travel_advance_requests.data.length > 0) {
                ...
                setTimeout(() => {
                  this.objectPrintAdvancesView.emit({
                    success: true,
                    data: [object],
                  });
                }, 500);
              ...
              if (detail.data[0].travel_managements.data.length > 0) {
                setTimeout(() => {
                  this.objectPrintTravelView.emit({
                    success: true,
                    data: [detail.data[0].travel_managements],
                  });
                }, 500);
              } else {
                setTimeout(() => {
  ...
  sedRequestsSpend() {
    this.spendsService.putSendRequestsSpend(this.id_spend).subscribe(
      (data: any) => {
        if (data) {
          document.getElementById('closeModalViewSpend').click();
  ...
  exportExcel() {
    this.spendsService
      .getExportSpendExcel(this.ticket)
      .subscribe((data: any) => {

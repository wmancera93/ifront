...
export class PdfTravelComponent implements OnInit {
  ...
  approvals(result, doc) {
    const { approvers_to_json, answers_to_json } = result.data[0].travel_request;
    if (approvers_to_json.length > 0) {
      const columnsHeaderJourneys = ['APROBADORES', 'ESTADOS', 'OBSERVACIONES'];
      const dataHeaderJourneys: any[] = [];
      let arrayPrint: any[];
      let index = 0;

      approvers_to_json.forEach(data => {
        if (answers_to_json[index] !== undefined) {
          arrayPrint = [
            data.short_name.toString(),
            answers_to_json[index].status.toString(),
            answers_to_json[index].observation.toString(),
          ];
        } else {

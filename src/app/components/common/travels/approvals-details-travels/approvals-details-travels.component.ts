...
export class ApprovalsDetailsTravelsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public accionDataTableService: DataDableSharedService,
    public travelsService: TravelsService,
    public spendSharedService: SpendSharedService,
    public translate: TranslateService,
  ) {
    this.nameReportTravel = this.t('travel_journeys');
    this.nameReportAdvance = this.t('travel_advances');
    this.nameReportSpend = this.t('travel_expenses');
    this.statusApprover = this.t('status_Approver');
    this.statusCancelled = this.t('status_cancelled');
    this.statusInProcess = this.t('status_in_Process');
    this.statusPending = this.t('status_pending');
    ...
    this.travelApproverServiceShared.getviewDetailRequests().subscribe((data: any) => {
      if (this.countAfter === 0) {
        ...
        switch (this.type_requests) {
          case 'travels':
            ...
            this.table_advances = [];
            this.table_spend = [];
            this.approverTravelsService.getApprovalsRequestsById(this.requests_travels.ticket).subscribe((request: any) => {
              if (request) {
          ...
          case 'advance':
            ...
            this.table_advances = [];
            this.table_spend = [];
            this.approverTravelsService.getApprovalsRequestsAdnvanceById(this.requests_travels.ticket).subscribe((advance: any) => {
          ...
          case 'spend':
            ...
            this.table_advances = [];
            this.table_spend = [];
            this.approverTravelsService.getApprovalsRequestsSpendById(this.requests_travels.ticket).subscribe((spend: any) => {
              if (spend) {
        ...
        this.approverTravelsService
          ...
          .subscribe(
            (data: any) => {
              ...
              if (data.success) {
                document.getElementById('btn_approvals_requests_travels').click();
                const alertWarning: Alerts[] = [{ type: 'success', title: this.t('type_alert_ts'), message: this.t('message_alert_ts'), confirmation: false }];
              ...
              const alertWarning: Alerts[] = [
                {
                  ...
                  message: error.json().errors.toString() + this.t('message_alert_one_ts'),
                  confirmation: true,
                  typeConfirmation: 'continueTravelRequestsApprover',
                },
              ];
              this.showSubmit = true;
              this.alert.setAlert(alertWarning[0]);
